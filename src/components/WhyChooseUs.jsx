        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reasons.map((item, index) => (
            <motion.div
              ...
              className="bg-white rounded-xl shadow-lg p-6 cursor-pointer border-t-4 transition-all duration-300 hover:shadow-2xl flex flex-col items-center"
            >
              {/* Icon */}
              <div
                className={`flex items-center justify-center w-16 h-16 mb-4 rounded-full ${item.color}`}
              >
                {item.icon}
              </div>
